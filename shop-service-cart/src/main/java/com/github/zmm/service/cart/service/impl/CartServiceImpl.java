package com.github.zmm.service.cart.service.impl;

import com.github.zmm.service.cart.service.api.entity.CartInfo;
import com.github.zmm.service.cart.service.api.entity.ShoppingCart;
import com.github.zmm.service.cart.service.api.model.ProductModel;
import com.github.zmm.service.cart.service.api.service.CartService;
import com.github.zmm.service.redis.RedisService;
import com.github.zmm.shop.common.constant.Constant;
...
public class CartServiceImpl implements CartService {
	...
	public boolean changeStatus(String userId, String productId) {
		String chooseProduct = chooseProduct(productId);
		String noChooseProduct = noChooseProduct(productId);
		if (redisService.exists(getCart(userId), chooseProduct)) {
			Long counts = (Long) redisService.getValueByKeyANdField(getCart(userId), chooseProduct);
			redisService.deleteField(getCart(userId), chooseProduct);
			redisService.put(getCart(userId), noChooseProduct, counts);
			return false;
		}else if (redisService.exists(getCart(userId), noChooseProduct)) {
			Long counts = (Long) redisService.getValueByKeyANdField(getCart(userId), noChooseProduct);
			redisService.deleteField(getCart(userId),noChooseProduct);
			redisService.put(getCart(userId), chooseProduct, counts);
			return true;
		}
		return false;
	}
